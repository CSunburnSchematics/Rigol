#!/usr/bin/env python3
"""
FINAL RADIATION TESTING SCRIPT - Multi-Scope 16-Channel Real-Time Capture
...
def capture_thread_for_scope(scope_info, config, data_queue, stop_event, stats, channel_names, output_dir):
    ...
    # Setup CSV file for this scope using custom output directory
    csv_filename = os.path.join(output_dir, f'multiscope_{serial}.csv')
...
def load_config(config_file):
    ...
    # First check if file exists as-is (could be absolute or relative path)
    ...
    # Check if it's an absolute path
        ...
        # Try just the basename in the configs folder
        basename = os.path.basename(config_file)
        config_path = os.path.join(os.path.dirname(__file__), "..", "configs", basename)
...
def main():
    print("="*70)
    print("FINAL RADIATION TESTING SCRIPT")
    ...
    if len(sys.argv) < 3:
        print("ERROR: Config file and output directory required!")
        print("Usage: python final_rad_testing_script.py <config_file> <output_directory>")
        print("Example: python final_rad_testing_script.py GAN_HV_TESTCONFIG.json C:/Users/andre/Claude/rad_test_data")
        return 1
    ...
    output_dir = os.path.join(os.path.abspath(output_base_dir), f'rad_test_{timestamp_str}')

    try:
        os.makedirs(output_dir, exist_ok=True)
        print(f"\nCreated timestamped output directory: {output_dir}")
    except Exception as e:
    ...
    for scope_info in scope_infos:
        ...
        thread = threading.Thread(
            ...
            args=(scope_info, capture_config, data_queue, stop_event, stats, scope_channel_names[scope_idx], output_dir),
            daemon=True
        )
        thread.start()
    ...
    fig.canvas.manager.set_window_title('FINAL RADIATION TEST - 16-Channel Multi-Scope Capture (Press Q to Quit)')
    ...
    # Save screenshot to custom output directory
    screenshot_filename = os.path.join(output_dir, 'multiscope_16ch_capture.png')
    try:
        print("\nSaving screenshot...")
    ...
    stats_output.append("FINAL RADIATION TEST - CAPTURE COMPLETE")
    stats_output.append("="*70)
    stats_output.append(f"Config: {config_file}")
    stats_output.append(f"Output Directory: {output_dir}")
    ...
    # Save stats to custom output directory
    stats_filename = os.path.join(output_dir, f'performance_{os.path.splitext(os.path.basename(config_file))[0]}.txt')
    try:
