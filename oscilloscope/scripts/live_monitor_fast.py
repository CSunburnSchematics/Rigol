#!/usr/bin/env python3
"""
Live oscilloscope monitor - FAST MODE
Captures whatever data is currently on screen without forcing new acquisitions
Updates plots in real-time as data streams in
"""
...
def connect_scope():
    ...
    scope.timeout = 3000  # Shorter timeout for fast mode
    return scope

def capture_waveform_fast(scope, channel=1):
    """Capture whatever is currently on screen - FAST MODE"""
    capture_start = datetime.now(timezone.utc)

    # Just read whatever's on screen - don't stop/start
...
class LiveMonitorFast:
    """Real-time oscilloscope monitor - FAST MODE"""
    ...
    def __init__(self, scope, channel=1, max_captures=100, csv_filename=None):
        ...
        self.total_points = 0
        self.all_voltages = deque(maxlen=10000)

        # Plot update control
        self.update_every = 5  # Update plot every 5 captures in fast mode
        self.last_plot_update = 0

        # CSV file for raw data
        if csv_filename is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            csv_filename = f'../../data/live_monitor_fast_{timestamp}.csv'
        ...
        self.fig.canvas.manager.set_window_title('Live Oscilloscope Monitor - FAST MODE')
    ...
    def capture_and_update(self):
        ...
        try:
            # Capture waveform (fast - just grab screen data)
            result = capture_waveform_fast(self.scope, self.channel)
            ...
            # Update plots every N captures to reduce lag
    ...
    def update_plots(self):
        ...
        # Only plot last 20 captures to keep it fast
        recent_captures = self.captures[-20:]
        ...
        self.ax1.set_xlabel('UTC Time')
        self.ax1.set_ylabel('Voltage (V)')
        self.ax1.set_title(f'Waveform Data on UTC Timeline (Last 20 of {len(self.captures)} captures)')
        self.ax1.grid(True, alpha=0.3)
        ...
        capture_rate = self.capture_count / elapsed if elapsed > 0 else 0
        status = (f"Captures: {self.capture_count} ({capture_rate:.1f}/s) | "
    ...
    def run(self, duration_sec=60, capture_interval=0.1):
        """Run live monitoring for specified duration"""

        print("="*70)
        print("LIVE OSCILLOSCOPE MONITOR - FAST MODE")
        print("="*70)
        print(f"Duration: {duration_sec} seconds")
        print(f"Capture interval: {capture_interval} seconds")
        print(f"Channel: {self.channel}")
        print("\nFAST MODE: Reading screen buffer without forcing new acquisitions")
        ...
        # Save final plot
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'../../plots/live_monitor_fast_{timestamp}.png'
...
def main():
    print("DS1054Z Live Monitor - FAST MODE\n")
    ...
    try:
        ...
        time.sleep(0.3)
        print("Trigger set to AUTO sweep")

        # Make sure scope is running
        scope.write(':RUN')
        time.sleep(0.3)
        print("Scope set to RUN mode\n")

        # Create and run live monitor in FAST mode
        monitor = LiveMonitorFast(scope, channel=1, max_captures=100)
        monitor.run(duration_sec=60, capture_interval=0.1)  # Fast! 100ms between captures
