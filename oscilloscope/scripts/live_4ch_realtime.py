...
def main():
    print("DS1054Z 4-Channel Real-Time Capture")
    print("="*70)
    ...
    for ch in range(1, 5):
        ...
        scope.write(f':CHANnel{ch}:PROBe 10')
        time.sleep(0.05)
        # Set voltage scale to 1V/div
        scope.write(f':CHANnel{ch}:SCALe 1.0')
        time.sleep(0.05)
        # Set offset to 0 first to clear any previous offset
    ...
    csv_filename = f'../../data/live_4ch_{timestamp_str}.csv'
    ...
    max_display_time = 10.0  # Show last 10 seconds
    max_points_per_channel = 10000  # Limit memory

    all_times = deque(maxlen=max_points_per_channel)
    all_data = {ch: deque(maxlen=max_points_per_channel) for ch in range(1, 5)}

    # Store last 10 complete waveforms for detail view
    last_waveforms = deque(maxlen=10)
            ...
            if data_processed and (current_time - last_update) > update_interval:
                if len(all_times) > 0:
                    ...
                    if len(all_voltages) > 0:
                        hist_ax.hist(all_voltages, bins=50, color='purple', alpha=0.7, edgecolor='black')
    ...
    timestamp_str = datetime.now().strftime('%Y%m%d_%H%M%S')
    screenshot_filename = f'../../plots/live_4ch_screenshot_{timestamp_str}.png'
    try:
