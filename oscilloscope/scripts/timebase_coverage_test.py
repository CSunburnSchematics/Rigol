#!/usr/bin/env python3
"""
Timebase coverage test - measure coverage for 100-point captures at different timebases
"""
...
def capture_100_points(scope, channel=1):
    """Capture 100 points"""
...
def test_timebase(scope, timebase_seconds, timebase_name, test_duration=20):
    ...
    print(f"Timebase set to: {float(actual)*1e9:.0f} ns/div")
    ...
    while time.time() - start_time < test_duration:
        result = capture_100_points(scope, channel=1)
...
def main():
    print("DS1054Z Timebase Coverage Test")
    print("="*70)
    print("Testing 100-point captures at different timebase resolutions")
    print("="*70)
    ...
    # Test different timebases
    # Format: (seconds_per_div, name)
    timebases = [
        (100e-9, "100ns"),
        (1e-6, "1us"),
        (10e-6, "10us"),
    ...
    # Summary table
    print(f"\n{'='*80}")
    print("SUMMARY - Timebase Coverage Analysis (100 points per capture)")
    print(f"{'='*80}")
    ...
    print(f"{'='*80}")
    print("\nKey insight: Faster timebase = shorter time span per capture")
    print("But capture rate stays roughly the same (~4.5/sec)")
    print("Coverage % = (total sample time) / (test duration)")
    print(f"{'='*80}")

    # Save detailed results
    timestamp_str = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_file = f'../../data/timebase_coverage_{timestamp_str}.txt'

    with open(results_file, 'w') as f:
        f.write("Timebase Coverage Test Results\n")
        f.write("="*80 + "\n\n")
